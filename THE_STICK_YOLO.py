...
found_arduino = False
try:
    ser = serial.Serial("COM7", 115200, timeout=None) #change port according to arduino
    time.sleep(2)
...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        if frame is not None:
            ...
            if len(centres) > -1:
                track_number = tracker_KF.trackIdCount
                # Track object using Kalman Filter
                tracker_KF.Update(centres,
                                  predicted_classes=predicted_classes,
                                  bounding_boxes=bounding_boxes)
                if track_number != tracker_KF.trackIdCount:
                    myScreenshot = pyautogui.screenshot(region=(150,150, 450, 500))
                    # change path according to folder
                    myScreenshot.save(r'C:\Users\tysze\OneDrive\Desktop\stuff\year 3\grp_proj\img test\ant_'+str(track_number+1)+r'.png')
                ...
                for i in range(len(tracker_KF.tracks)):
                    if len(tracker_KF.tracks[i].trace) > 1:
                        ...
                        for j in range(len(tracker_KF.tracks[i].trace) - 1):
                            ...
                            if found_arduino:
                                if tracker_KF.ant_presence==True:
                                    ant_presence = "yes"
                                    ser.write(b"1")
                                else:
                                    ant_presence = "no"
